...
{
    public partial class fmSettingPointBendingConcave : DevExpress.XtraEditors.XtraForm
    {
        System.Windows.Forms.Timer t;

        public fmSettingPointBendingConcave()
        {
            InitializeComponent();
        ...
        {
            if (!SQLite.Instance().CheckExistData("T24DefaultPointBendingConcave", VarGlobal.Point + 1))
            {
                //MessageBox.Show("Empty");
                //return;
                SQLite.Instance().InsertPointsCoordinateBending("T24DefaultPointBendingConcave", VarGlobal.Point + 1,
                "Point " + (VarGlobal.Point + 1), lblCoorX.Text, lblCoorY.Text,lblCoorZ.Text);
                this.Close();
            }
            else
            {
                SQLite.Instance().UpdatePointsCoordinateBending("T24DefaultPointBendingConcave", VarGlobal.Point + 1,
                    lblCoorX.Text, lblCoorY.Text, lblCoorZ.Text);
                this.Close();
            }
        }

        private void SettingPointBendingConcave_FormClosing(object sender, FormClosingEventArgs e)
        {
            t.Enabled = false;
        ...
        private void fmSettingPointBendingConcave_Load(object sender, EventArgs e)
        {
            string[] _arr = SQLite.Instance().GetPointsCoordinate("T24DefaultPointBendingConcave",
